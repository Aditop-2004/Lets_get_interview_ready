...
let joinStream = async () => {
  try {
    ...
    localTracks = await AgoraRTC.createMicrophoneAndCameraTracks(
      {},
      {
        //for improving the camera quality
        // encoderConfig: {
...
let handleUserLeft = async (user) => {
  ...
  //if the user that leaves the room is in the display frame,we must remove the display and all the other users will be norally displayed in circles but in little bigger boxes.
...
//for togglling the mic using the mic access control button
let toggleMic = async (e) => {
  let button = e.currentTarget;
  //if mic is off ,we will turn it on
...
//for togglling the camera using the camera access control button
let toggleCamera = async (e) => {
  let button = e.currentTarget;
  //if camera is off ,we will turn it on
